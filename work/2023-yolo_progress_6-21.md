...
NOTE: As of this document being uploaded, it has been untested, both in instruction and in visualization. Despite working once, I do not know if it works on repeat, or on accounts that are not my own, given the requirement of an API key that may be generally linked to me. This list of instructions also needs to be rewritten in general for clarity and conciseness.
...
How to Create a YOLOv5 Model with Google Colab:
...
1. Open the [Google Colab file here](https://colab.research.google.com/github/ultralytics/yolov5/blob/master/tutorial.ipynb) from the GitHub. If you'd wish to keep it, save a personal copy into your Google Drive. This will be the baseline used to 
...
(The API Key above is from my dataset in Roboflow. As it is based in the research that is built to be semi-private, I have no qualms against posting it here.)
...
```
!python train.py --img (size of image) --batch (number of iamges) --epochs (number of trials) --data (name of dataset).yaml --weights (weight) --cache (type of cache, ram or disk)
```
...
I have less to talk about most of this week, as most of it was spent annotating data in Roboflow to have more data. More data leads to more accurate models, but I do have things to say about how my annotation style has changed.
...
(Current Progress as of the time stamp at which this file was posted: 119 registered annotated images)
